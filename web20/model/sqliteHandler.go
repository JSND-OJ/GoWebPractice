...
func (s *sqliteHandler) GetTodos() []*Todo {
	todos := []*Todo{}
	rows, err := s.db.Query("SELECT id, name, completed, createdAt FROM todos")//Query executes a query that returns rows
	if err != nil {
		panic(err)
	}
	defer rows.Close()
...
func (s *sqliteHandler) CompleteTodo(id int, complete bool) bool {
	...
	cnt, _ := rst.RowsAffected()// RowsAffected returns the number of rows affected by an update, insert, or delete.
	return cnt > 0
}
