...
func TestCreateUser(t *testing.T) {
	...
	ts := httptest.NewServer(NewHandler()) //app.go 뉴핸들러 호출
	defer ts.Close()

	resp, err := http.Post(ts.URL+"/users", "application/json",
		strings.NewReader(`{"first_name":"Jungehon", "last_name":"OH", "email":"cuttleoh@naver.com"}`)) // ``go가 이해할수 있도록 어노테이션
	assert.NoError(err)
	...
	resp, err = http.Get(ts.URL + "/users/" + strconv.Itoa(id)) //Itoa 정수형을 문자열로 변환. id정보를 get 방식으로 user/id를 넣어서 오도록 만든다.get방식 쓰는 이유? 저장된 값들을 확인할려고
	assert.NoError(err)
	assert.Equal(http.StatusOK, resp.StatusCode)
